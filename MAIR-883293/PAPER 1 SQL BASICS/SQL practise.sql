-- 1. (Easy) List every order with customer's full name, store name, and staff full name
SELECT 
    o.order_id,
...
-- 2. (Easy) Show each product with brand name and category name (LEFT JOIN for unassigned)
SELECT 
    p.product_id,
    p.product_name,
    b.brand_name,
...
-- 8. (Medium) Customers with more orders than overall customer order average
SELECT 
    c.customer_id,
    c.first_name + ' ' + c.last_name AS customer_name,
...
/*
Readability Note:
CTE version code ko divide kar ke structured banata hai jisse average logic aur customer group logic alag samajh aate hain. Subquery simple cases mein concise hoti hai, lekin CTE modular analysis ke liye best hai.
*/
